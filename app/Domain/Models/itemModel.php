...
class ItemModel extends BaseModel
{
    ...
    {
        // code
        $sql = "SELECT i.* c.category_name,u.username FROM items i
        JOIN category c ON i.category_id = c.category_id
        JOIN users u ON i.user_id = u.user_id
        WHERE i status = 'Available' ORDER BY i.listing_date DESC";
    ...
    public function searchItems(string $search): array
    {
        ...
        $sql = "SELECT i.*, c.category_name,u.username FROM items i
            JOIN category c ON i.category_id =c.category_id
            JOIN users u ON i.user_id = u.user_id
            WHERE i.status = 'Available' AND i.listing_product LIKE CONCAT('%', :search, '%')
            ORDER BY i.listing_date DESC";
        $stmt = $this->pdo->prepare($sql);
